...
const assertSoloLayout = async ({
  ...
  await expect(page.getByTestId("work-grid").first()).toBeVisible();
...
for (const viewport of GAMEPLAY_VIEWPORTS) {
  test.describe(`solo gameplay layout ${viewport.name}`, () => {
    ...
    for (const cards of GAMEPLAY_CARD_COUNTS) {
      for (const variant of GAMEPLAY_REVEALED_VARIANTS) {
        test(`reveal layout cards=${cards} variant=${variant}`, async ({ page }) => {
          ...
          await assertSoloLayout({ page, cards, viewportWidth: viewport.width });

          await expect(page).toHaveScreenshot(
            `gameplay-solo-${viewport.name}-cards-${cards}-${variant}-revealed.png`,
            {
              fullPage: true,
              animations: "disabled",
              maxDiffPixels: 120,
    ...
    for (const variant of GAMEPLAY_STATE_VARIANTS) {
      for (const state of GAMEPLAY_STATES_FOR_COUNT_FIVE) {
        test(`state layout cards=5 variant=${variant} state=${state}`, async ({ page }) => {
          ...
          await assertSoloLayout({ page, cards: 5, viewportWidth: viewport.width });

          await expect(page).toHaveScreenshot(
            `gameplay-solo-${viewport.name}-cards-5-${variant}-${state}.png`,
            {
              fullPage: true,
              animations: "disabled",
              maxDiffPixels: 120,
