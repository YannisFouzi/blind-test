...
import { usePartyKitLobby } from "@/features/multiplayer-game/hooks/usePartyKitLobby";
...
export const HomeContent = () => {
  ...
  // Hook PartyKit Lobby: ne connecter qu'en mode multi (evite une connexion inutile en solo)
  const {
    rooms: partyKitRooms,
  ...
  // Mode custom: toutes les oeuvres de tous les univers
  ...
  // Adapter PartyKit RoomMetadata vers le type Room pour compatibilite JSX
  ...
  // Charger/initialiser l'identite persistee (playerId + pseudo)
  useEffect(() => {
  ...
  const handleCreateRoom = useCallback(async () => {
    ...
    try {
      ...
      // PartyKit: generation d'ID locale (instantanee)
      ...
      navigate(targetUrl);
      // Note: setHomeInfo ne sera jamais affiche car la page est immediatement rechargee
    } catch (error) {
  ...
  const handleJoinRoom = useCallback(
    async (roomId: string, password?: string) => {
      ...
      // Connexion directe a la room via la page d'attente
      setHostRoomId("");
  ...
  const handleUniverseClick = useCallback(
    async (universeId: string) => {
      ...
      setIsCreatingRoom(true);
      try {
        // PartyKit gere la configuration via WebSocket dans GameClient
  ...
  const applyCustomizeAndPlay = useCallback(async () => {
    ...
    // FIX: sauvegarder les valeurs avant d'appeler closeCustomize(), qui reinitialise tout
    ...
    // Un seul bouton Mode Custom: le toggle dans la modal choisit personnalise ou aleatoire
    ...
    if (mode === "solo") {
      ...
      // Effets mysteres (solo) - utiliser les valeurs sauvegardees
      ...
      // Mode aleatoire solo: nombre d'oeuvres par manche (2-8)
    ...
    if (!isHost || !hostRoomId) {
      setHomeError("Seul l'host peut appliquer les parametres personnalises.");
      return;
    }
    ...
    try {
      ...
      // Passer les works selectionnes si necessaire
      if (savedAllowedWorks.length > 0) {
        params.set("works", savedAllowedWorks.join(","));
      }

      // Passer le nombre max de songs si specifie
      if (savedMaxSongs !== null) {
        params.set("maxsongs", String(savedMaxSongs));
      }
      hasUsedRoomRef.current = true;
            ...
            <div className="magic-card p-6 md:p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1.4fr)] md:items-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  >
                    {hostRoomId ? "Room creee" : isCreatingRoom ? "Creation..." : "Creer une room"}
                  </button>
                  <input
                    type="password"
                        ...
                        >
                          <div className="flex flex-col gap-1 min-w-[140px]">
                            <div className="flex items-center gap-2 text-sm font-semibold text-[var(--color-text-primary)]">
                              ...
                              <span>Hote: {room.hostName || "Inconnu"}</span>
                              {room.hasPassword && (
                                <span className="text-[10px] uppercase tracking-[0.2em] text-[#B45309]">Protegee</span>
                              )}
                            </div>
                          </div>
                          {room.hasPassword && (
