...
export class YouTubeService {
  ...
  static extractPlaylistId(url: string): string | null {
    return extractIdFromPatterns(url, PLAYLIST_PATTERNS);
  }

  static extractVideoId(url: string): string | null {
    return extractIdFromPatterns(url, VIDEO_PATTERNS);
  }

  static async validateVideo(videoUrl: string) {
    const videoId = this.extractVideoId(videoUrl);
    if (!videoId) {
      return { isValid: false, error: "ID de video introuvable" };
    }
    ...
    try {
      ...
      if (!response.success || !response.data) {
        return {
          isValid: false,
          error: response.error || "Video introuvable",
        };
      }
      ...
      return videoValidationResultSchema.parse({
        ...
        duration: response.data.duration
          ? parseIsoDuration(response.data.duration)
          : undefined,
      });
    } catch (error) {
      return {
